...
* **SQLLogicTest Timing Triage:** Use sqllogictest timing controls to find slow files in CI/non-TTY runs: `--timing-summary {auto|off|top|full}`, `--timing-top-n <N>`, `SLT_TIMING_SUMMARY`, `SLT_TIMING_TOP_N`; optional debug logging uses `SLT_TIMING_DEBUG_SLOW_FILES=1`.
