...
This leaves three implementations of the same Java/Spark-visible sign-and-width rules. A prior decimal formatting bug omitted the negative-parentheses suffix because equivalent final assembly had drifted between numeric paths. The integer path can drift independently for `(`, `+`, space, `0`, `-`, and width combinations.
...
For integer `%d`, the result must remain compatible with the existing Java/Spark contract for signed and unsigned values, grouping, `+`, space, `(`, `0`, `-`, and width.
...
- Add regression coverage for the shared final-assembly contract across float, decimal, signed integer, and unsigned integer inputs.
...
- [ ] `%d` preserves correct output for positive signed and unsigned values with `+`, space, `0`, `-`, grouping, and width.
...
- [ ] No duplicate final width-padding branches remain for decimal integer formatting.
...
- Add SQLLogicTest coverage in `datafusion/sqllogictest/test_files/spark/string/format_string.slt` for SQL-visible representative cases:
  ...
  - positive/negative `%d` with `+` or space and width;
  - unsigned `%d` with zero padding and width;
...
- Confirm Java `Formatter` output for the selected combined flag cases before recording expected values; this issue must preserve the compatibility contract rather than infer it from the current implementation.
