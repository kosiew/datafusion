...
> **Note:** Comparisons involving `NaN` are unordered. Any comparison with `NaN` (such as `NaN > 1` or `1 <= NaN`) evaluates to `false`. The `greatest` function returns `NaN` if any argument is `NaN`, while `least` ignores `NaN` values in favor of other inputs.
