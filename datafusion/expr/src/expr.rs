...
/// let expr = col("c1");
/// assert_eq!(expr, Expr::Column(Box::new(Column::from_name("c1"))));
/// ```
///
...
impl Expr {
    ...
    pub fn add_column_ref_counts<'a>(&'a self, map: &mut HashMap<&'a Column, usize>) {
        self.apply(|expr| {
            if let Expr::Column(col) = expr {
                *map.entry(col.as_ref()).or_default() += 1;
            }
            Ok(TreeNodeRecursion::Continue)
        })
        .expect("traversal is infallible");
...
mod test {
    ...
    fn test_display_set_comparison() {
        ...
        let expr = Expr::SetComparison(SetComparison::new(
            Box::new(Expr::Column(Box::new(Column::from_name("a")))),
            subquery,
            Operator::Gt,
            SetQuantifier::Any,
        ));
