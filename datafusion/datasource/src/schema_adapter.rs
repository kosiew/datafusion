...
use datafusion_common::{
    ...
    plan_err, ColumnStatistics, Result,
};
...
/// use arrow::datatypes::DataType::{self, Float64, Int32, Utf8};
        ...
        let (field_mappings, projection) = create_field_mapping(
            ...
            |file_field, table_field| {
                ...
                match (file_field.data_type(), table_field.data_type()) {
                    ...
                    _ => can_cast_field(file_field, table_field),
                }
            },
        )?;
...
mod tests {
    ...
    use arrow::{
        ...
        compute::cast,
        datatypes::{
            DataType::{self, Binary, Decimal128, Float64, Int32, Int64, Struct, Timestamp, Utf8},
    ...
    use datafusion_common::{
        stats::{Precision, Statistics},
        ColumnStatistics, Result, ScalarValue,
    };
    ...
    fn test_can_cast_field() {
        ...
        let from_field = Field::new("col", Int32, true);
        let to_field = Field::new("col", Int32, true);
        assert!(can_cast_field(&from_field, &to_field).unwrap());

        // Casting Int32 to Float64 is allowed
        let from_field = Field::new("col", Int32, true);
        let to_field = Field::new("col", Float64, true);
        assert!(can_cast_field(&from_field, &to_field).unwrap());

        // Casting Float64 to Utf8 should work (converts to string)
        let from_field = Field::new("col", Float64, true);
        let to_field = Field::new("col", Utf8, true);
        assert!(can_cast_field(&from_field, &to_field).unwrap());
        ...
        let from_field = Field::new("col", Binary, true);
        let to_field = Field::new("col", Decimal128(10, 2), true);
    ...
    fn test_create_field_mapping() {
        ...
        let file_schema = Schema::new(vec![
            Field::new("b", Float64, true), // Different type but castable to Utf8
            Field::new("a", Int32, true),   // Same type
    ...
    fn test_schema_mapping_new() {
        ...
        let batch = RecordBatch::try_new(
            Arc::new(Schema::new(vec![
                Field::new("b_file", Utf8, true),
                Field::new("a_file", Int32, true),
            ])),
            vec![
    ...
    fn test_map_schema_error_path() {
        ...
        let table_schema = Arc::new(Schema::new(vec![
            Field::new("a", Int32, true),
            Field::new("b", Utf8, true),
            Field::new("c", Decimal128(10, 2), true), // Use Decimal which has stricter cast rules
        ]));

        // Define file schema with incompatible type for column c
        let file_schema = Schema::new(vec![
            Field::new("a", Int32, true),
            Field::new("b", Float64, true), // Different but castable
            Field::new("c", Binary, true),  // Not castable to Decimal128
        ]);
    ...
    fn test_map_schema_happy_path() {
        ...
        let table_schema = Arc::new(Schema::new(vec![
            Field::new("a", Int32, true),
            Field::new("b", Utf8, true),
            Field::new("c", Decimal128(10, 2), true),
        ]));
        ...
        let compatible_file_schema = Schema::new(vec![
            Field::new("a", Int64, true), // Can be cast to Int32
            Field::new("b", Float64, true), // Can be cast to Utf8
        ]);
    ...
    fn test_adapter_factory_selection() -> Result<()> {
        ...
        // Default adapter should handle schema evolution
        assert!(
            default_adapter.map_schema(&source_schema).is_ok(),
            "Default adapter should handle schema with missing fields"
        );
    ...
    fn test_adapt_struct_with_added_nested_fields() -> Result<()> {
        ...
        let mapped_batch =
            adapt_batch_with_default_schema_adapter(&file_schema, &table_schema, batch)?;
    ...
    /// Apply the default schema adapter to the batch
    fn adapt_batch_with_default_schema_adapter(
