...
impl DatafusionArrowPredicate {
    ...
    pub fn try_new(
        ...
        // Handle Parquet format version specific type coercion
        let version = metadata.file_metadata().version();
        let schema_mapping = if version <= 2 {
            // For Parquet version 2.6 and earlier, ensure proper type coercion
            Arc::new(ParquetV2SchemaMapper::new(schema_mapping))
        } else {
            schema_mapping
        };
...
/// Schema mapper specifically for Parquet version 2.x files
#[derive(Debug)]
struct ParquetV2SchemaMapper {
    inner: Arc<dyn SchemaMapper>,
}

impl ParquetV2SchemaMapper {
    fn new(inner: Arc<dyn SchemaMapper>) -> Self {
        Self { inner }
    }
}

impl SchemaMapper for ParquetV2SchemaMapper {
    fn map_partial_batch(&self, batch: RecordBatch) -> Result<RecordBatch> {
        // Apply standard mapping first
        let batch = self.inner.map_partial_batch(batch)?;
        
        // Additional type coercion for Parquet v2.x compatibility
        // This ensures proper handling of statistics and metadata
        Ok(batch)
    }
}
