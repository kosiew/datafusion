...
impl AggregateFunctionExpr {
    ...
    /// available, even though the effective argument fields can be derived from
    /// [`Self::input_fields`]. Physical expressions may reference columns that
    /// are not one-to-one with the aggregate arguments (for example,
    /// `SUM(a + b)` references both `a` and `b`). Replacing the schema with the
    /// synthesized argument fields would prevent those expressions from
